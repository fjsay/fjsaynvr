...
# V1.0: 初始版本
# V3.0: 增加对TS视频格式的支持
# V3.1: 修复监控丢失后录制卡住的问题，增加断线自动重连功能
import ffmpeg
import time
import os
        ...
        with open(file_path, 'r') as f:
            for line in f:
                ...
                if line and not line.startswith('#'):  # 跳过空行和注释
                    parts = line.split(',')
                    # 配置格式：ID, RTSP地址, 保留天数, 录制时长(秒), 保存格式(mp4/flv/ts), 最小文件大小(KB)
                    if len(parts) >= 4:
...
def clean_expired_recordings(camera_id, retention_days):
    ...
    try:
        ...
        # 递归遍历目录查找所有视频文件（增加ts格式支持）
        for root, _, files in os.walk(camera_root_dir):
            for filename in files:
                if filename.endswith(('.mp4', '.flv', '.ts')):
...
# 添加流检测函数
def check_rtsp_stream(rtsp_url, timeout=60):
    """检测RTSP流是否可用"""
    try:
        # 使用ffprobe快速检测流
...
def record_video(camera_info):
    ...
    while True:
        # 录制前先检测流是否可用
        stream_available = check_rtsp_stream(rtsp_url)
        if not stream_available:
            consecutive_failures += 1
            logging.warning(
                f"摄像头 {camera_id} 流不可用，将重试 "
                f"(RTSP地址: {rtsp_url}, 上次成功时间: {last_success_time}, 连续失败: {consecutive_failures})"
            )
            
            # 指数退避策略：连续失败次数越多，等待时间越长
            wait_time = min(60, 2 ** consecutive_failures)
            time.sleep(wait_time)
            continue
        
        # 流可用，重置连续失败计数
        if consecutive_failures > 0:
            logging.info(f"摄像头 {camera_id} 已恢复连接，连续失败次数重置")
        ...
        try:
            ...
            # 设置超时时间（比录制时长多10秒，避免正常录制被误杀）
            timeout = record_duration + 10
            ...
            while process.poll() is None:
                if time.time() - start_time > timeout:
                    ...
                    if process.poll() is None:
                        process.kill()  # 强制终止
                    logging.warning(f"摄像头 {camera_id} 录制超时，已强制终止进程")
                    break
                time.sleep(0.5)  # 每0.5秒检查一次

            # 处理进程返回结果
            if process.returncode == 0:
                logging.info(f"摄像头 {camera_id} 录制成功：{output_file}")
                last_success_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                ...
                logging.error(
                    f"摄像头 {camera_id} 录制失败，返回码：{process.returncode} "
                    f"(RTSP地址: {rtsp_url}, 上次成功时间: {last_success_time})"
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 确保所有ffmpeg进程都被终止
        logging.info("正在清理残留的ffmpeg进程...")
        for proc in psutil.process_iter(['name', 'cmdline']):
            try:
                if 'ffmpeg' in proc.info['name'].lower():
                    # 只终止当前程序启动的ffmpeg进程（通过命令行参数判断）
                    cmdline = proc.info['cmdline'] or []
                    if any(rtsp_url in arg for arg in cmdline for rtsp_url in [c['url'] for c in cameras]):
                        proc.terminate()
                        ...
                        if proc.is_running():
                            proc.kill()
                            logging.info(f"已强制终止ffmpeg进程: {proc.pid}")
            except Exception as e:
